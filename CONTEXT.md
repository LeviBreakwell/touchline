...
A Fixture may be a **final**. Spawtz labels these — "Semi Final 1", "Final" — in an extra leading cell, so a finals row has six cells where an ordinary one has five. ⚠️ **The scraper does not account for that today**, and it does not fail safely: `Time.parse` accepts the shifted string instead of rejecting it, so every finals match is stored at midnight with the **court name as the opponent** and no score, unverifiable forever (#27).
